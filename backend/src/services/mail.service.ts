...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  sendSmtpEmail.subject = "Verify your Manufiq account";
  sendSmtpEmail.htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center; color: white;">
        <h1 style="margin: 0; font-size: 28px;">Welcome to Manufiq!</h1>
      </div>
      <div style="padding: 30px; background: #f9f9f9;">
        ...
        <p style="color: #888; font-size: 12px; margin-top: 25px;">
          If you didn't create an account with Manufiq, please ignore this email.
        </p>
      </div>
      <div style="background: #333; padding: 20px; text-align: center; color: #999; font-size: 12px;">
        <p style="margin: 0;">&copy; 2025 Manufiq. All rights reserved.</p>
      </div>
    </div>
  `;
...
export async function sendPasswordResetEmail(email: string, token: string) {
  try {
    ...
    sendSmtpEmail.sender = {
      name: "Manufiq",
      email: "danieldejen23@gmail.com" // Consider using a verified domain
    };
